...
staticcontent.combo.Context = function(config) {
    config = config || {};
    ...
    if (config.custm) {
        config.triggerConfig = [{
            ...
            cn: [{
                tag: 'div',
                cls: 'x-form-trigger x-field-search-context-go'
            }]
        }];
        if (config.clear) {
            config.triggerConfig[0].cn.push({
                tag: 'div',
                cls: 'x-form-trigger x-field-search-context-clear'
            });
        }
    ...
    Ext.applyIf(config, {
        name: config.name || 'context_key',
        hiddenName: config.name || 'context_key',
        displayField: 'name',
        valueField: 'key',
        editable: true,
        fields: ['name', 'key'],
    ...
    staticcontent.combo.Context.superclass.constructor.call(this, config);

};
Ext.extend(staticcontent.combo.Context, MODx.combo.ComboBox);
Ext.reg('staticcontent-combo-context', staticcontent.combo.Context);
